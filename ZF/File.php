...
class ZF_File
{
	...
	*/
	...
	public static function load($file)
	{
		...
		if (!self::$_file)
		{
			throw new ZF_Exception('File does not exist or is not readable: ' . $file);
		}

		if (!is_readable(self::$_file))
		{
			throw new ZF_Exception('File does not readable: ' . $file);
		}
	...
	function copyDir($source, $dest, $overwrite = false)
	{
		if (!is_dir($dest)) {
			if (!is_writable(dirname($dest))) {
				throw new ZF_Exception('filesystem not writable:' . dirname($dest));
			}
			mkdir($dest);
		}
					...
					if (self::exists($path)) {
						if (!self::exists($dest . '/' . $file) || $overwrite) {
							if (!@copy($path, $dest . '/' . $file)){
								throw new ZF_Exception('filesystem not writable:' . $dest . '/' . $file);
							}
						}
					} elseif (is_dir($path)) {
						if (!is_dir($dest . '/' . $file)) {
							if (!is_writable(dirname($dest . '/' . $file))) {
								throw new ZF_Exception('filesystem not writable:' . dirname($dest . '/' . $file));
							}
