<?php
abstract class ZF_Abstract_Pdo extends ZF_Abstract_DBOlder
{
	/**
	* 当前数据库链接
	* 
	...
	*/
	...
	public function getSqlComment(){
        if(!$this->sqlComment){
            ...
            $typeStr = "WEB";
            $isCli = false;
            switch(ZF_Request::resolveType()){
                case ZF_Request::CLI:
                    $typeStr = "CLI";
                    $isCli   = true;
                    break;
                case ZF_Request::AJAX:
                    $typeStr = "AJAX";
                    break;
                case ZF_Request::BROWSER:
                default:
                    $typeStr = "WEB";
                    break;
            }
	...
	*/
	...
	protected function chooseDbConn($sql)
	{
		...
		if (empty($this->db)) {
			if (empty($this->db)) {
				throw new ZF_Exception('Dose not exist instance of DB server!');
			}
		} else {
			$this->reconnectNum = 0;
		}
