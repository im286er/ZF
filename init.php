...
class ZF
{
	...
	*/
	...
	|---------------------------------------------------------------
	| @param string $name A ZF (or other) class or interface name.
	| @return void
	*/
	public static function autoload($name)
	{
		if (trim($name) == '')
		{
			new ZF_Exception('No class or interface named for loading');
		}
		...
		{
			throw new ZF_Exception("The namespace config have problem: '{$name}'");
		}

		if (! file_exists($file))
		{
			throw new ZF_Exception("The file dose not exist: '{$file}'");
		}
		if (! is_readable($file))
		{
			throw new ZF_Exception("The file can not read: '{$file}'");
		}
		...
		if (! class_exists($name, false) && ! interface_exists($name, false))
		{
			throw new ZF_Exception('Class or interface does not exist in loaded file');
		}
	...
	{
		if (empty($path)) {
			throw new ZF_Exception('No class or interface named for loading');
		}
